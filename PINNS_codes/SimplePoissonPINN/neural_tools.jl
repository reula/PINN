...
function generate_input0(config)
    @unpack N_points0, N_points, xmin, xmax, tmin, tmax = config
    ...
    input = vcat(x, t) |> gpu_device() .|> Float64
    #input = vcat(x, t) |> gpu_device() .|> Float32
...
function calculate_Dirichlet_f(x, t, NN, Θ, st)
    ...
    return u0 - ((t .- tmin).^2) ./ (1.0 .+ (t .- tmin).^2) .* (x .- xmax) .* (x .- xmin) .* nn_out 
end
...
function calculate_derivatives_Dirichlet(x, t, NN, Θ, st)
    ...
    f      = calculate_Dirichlet_f(x, t, NN, Θ, st)
    fxp    = calculate_Dirichlet_f(x .+ ϵ, t, NN, Θ, st)
    fxm    = calculate_Dirichlet_f(x .- ϵ, t, NN, Θ, st)
    ftp    = calculate_Dirichlet_f(x, t .+ ϵ, NN, Θ, st)
    ftm    = calculate_Dirichlet_f(x, t .- ϵ, NN, Θ, st)
